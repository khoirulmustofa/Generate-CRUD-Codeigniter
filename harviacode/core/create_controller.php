...
$string .= "\n\n    public function index()
    {
        ...
        \$config['total_rows'] = \$this->" . $m . "->total_rows_$c_url(\$q);
        \$$c_url = \$this->" . $m . "->get_limit_data_$c_url(\$config['per_page'], \$start, \$q);
...
$string .= "\n\n    public function ".$c_url."_read(\$id) 
    {
        \$row = \$this->" . $m . "->get_by_id_$c_url(\$id);
        if (\$row) {
            \$data = array(";
foreach ($all as $row) {
...
$string .= "\n\t    );
    ...
    public function ".$c_url."_create() 
    {
        \$data = array(
            'button' => 'Create',
            'action' => site_url('$c_url/".$c_url."_create_action'),";
foreach ($all as $row) {
...
$string .= "\n\t);
    ...
    public function ".$c_url."_create_action() 
    {
        \$this->".$c_url."_rules();
...
$string .= "\n\t    );
            ...
            \$this->".$m."->insert_$c_url(\$data);
            \$this->session->set_flashdata('message', 'Create Record Success');
            redirect(site_url('$c_url'));
        }
    }
    
    public function ".$c_url."_update(\$id) 
    {
        \$row = \$this->".$m."->get_by_id_$c_url(\$id);
        ...
        if (\$row) {
            \$data = array(
                'button' => 'Update',
                'action' => site_url('$c_url/".$c_url."_update_action'),";
foreach ($all as $row) {
...
$string .= "\n\t    );
    ...
    public function ".$c_url."_update_action() 
    {
        \$this->".$c_url."_rules();

        if (\$this->form_validation->run() == FALSE) {
            \$this->".$c_url."_update(\$this->input->post('$pk', TRUE));
        } else {
            \$data = array(";
foreach ($non_pk as $row) {
    $string .= "\n\t\t'" . $row['column_name'] . "' => \$this->input->post('" . $row['column_name'] . "',TRUE),";
...
$string .= "\n\t    );
            ...
            \$this->".$m."->update_$c_url(\$this->input->post('$pk', TRUE), \$data);
            \$this->session->set_flashdata('message', 'Update Record Success');
            redirect(site_url('$c_url'));
        }
    }
    
    public function ".$c_url."_delete(\$id) 
    {
        \$row = \$this->".$m."->get_by_id_$c_url(\$id);

        if (\$row) {
            \$this->".$m."->delete_$c_url(\$id);
...
/* Created by Khoirul Mustofa ".date('Y-m-d H:i:s')." */";
