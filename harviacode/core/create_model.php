...
$string .="\n\n    // get all
    ...
    // get data by id
    function get_by_id_$table_name(\$id)
    {
    ...
    // get total rows
    function total_rows_$table_name(\$q = NULL) {
        \$this->db->like('$pk', \$q);";
...
$string .= "\n\t\$this->db->from(\$this->table);
    ...
    function get_limit_data_$table_name(\$limit, \$start = 0, \$q = NULL) {
        \$this->db->order_by(\$this->id, \$this->order);
        \$this->db->like('$pk', \$q);";
...
$string .= "\n\t\$this->db->limit(\$limit, \$start);
    ...
    // insert data
    function insert_$table_name(\$data)
    {
        \$this->db->insert(\$this->table, \$data);
    }

    // update data
    function update_$table_name(\$id, \$data)
    {
        \$this->db->where(\$this->id, \$id);
